...
double mainthread(int *d_words, int *d_wordsXthread, int *array_len, int *d_results) {

    int *h_results = new int[MAX_ELEM];
    cudaMemset(d_results, 0, sizeof(int) * MAX_ELEM);
    ...
    cudaMemcpy(h_results, d_results, sizeof(int) * MAX_ELEM, cudaMemcpyDeviceToHost);
...
void benchmark(int totlen, int variabilty, int warps, int n_iter) {
    ...
    cudaMalloc((void**)&d_results, sizeof(int) * MAX_ELEM);
    ...
    NTHREAD = 0;
    fstream outputAll;
    outputAll.open("output_v4_b"+to_string(NBLOCK)+"_n_"+to_string(totlen)+"_all.txt", ios::out);
    for(int i = 0; NTHREAD < 1024; i++) {
        ...
        for(int j = 0; j < n_iter; j++) {
            cout<<"Run: "<<j+1<<endl;
            auto tmp = mainthread(d_words, d_wordsXthread, array_len, d_results);
            outputAll<<tmp<<endl;
            avg[0] += tmp/n_iter;
        }
    ...
    fstream output;
    output.open("output_v4_b"+to_string(NBLOCK)+"_n_"+to_string(totlen)+".txt", ios::out);
