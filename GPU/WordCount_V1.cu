...
__global__ void thread_func(int *words_int, int *nwords, int *var, int *result, int *len) {
    ...
    for(int i = start; i < end; i++) {
        if(i < size) result[tid*range+words_int[i]] += 1;
    }
...
vector<double> mainthread(int *d_words, int *totlen, int *d_wordsXthread, int *var) {
    ...
    int *h_results = new int[MAX_ELEM * NBLOCK * NTHREAD], *d_results;
    unordered_map<int, int>res;

    cudaMalloc((void**)&d_results, sizeof(int) * MAX_ELEM * NBLOCK * NTHREAD);
    cudaMemset(d_results, 0, sizeof(int) * MAX_ELEM * NBLOCK * NTHREAD);
    ...
    cudaMemcpy(h_results, d_results, sizeof(int) * MAX_ELEM * NBLOCK * NTHREAD, cudaMemcpyDeviceToHost);

    //merge results from threads
    for(int i = 0; i < NBLOCK * NTHREAD; i++) {
        for(int j = 0; j < MAX_ELEM; j++) {
            if(h_results[i*MAX_ELEM+j] != 0) res[j] += h_results[i*MAX_ELEM+j];
        }
    }
...
void benchmark(int totlen, int variabilty, int warps, int n_iter) {
    ...
    fstream output;
    output.open("output_v1_b"+to_string(NBLOCK)+"_n_"+to_string(totlen)+".txt", ios::out);
